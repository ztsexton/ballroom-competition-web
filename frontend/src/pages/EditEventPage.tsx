...
const EditEventPage = () => {
  ...
  const handleSubmit = async (e: FormEvent, forceOverwrite = false) => {
    ...
    if (!originalEvent || !activeCompetition) return;
  ...
  if (!isAdmin) {
    return (
      <div className="container">
        <div className="card">
          <h2>Access Denied</h2>
          <p>You must be an admin to edit events.</p>
        </div>
      </div>
    );
  }

  if (!activeCompetition || !originalEvent) {
    return (
      <div className="container">
        <div className="card">
    ...
    <div className="container">
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h2>Edit Event - {activeCompetition.name}</h2>
          <button onClick={() => navigate(`/events/${id}`)} className="btn btn-secondary">Cancel</button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <input
              ...
              placeholder="e.g., Bronze Waltz, Silver Foxtrot"
              required
            />
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1.5rem' }}>
            ...
            <button type="button" onClick={() => navigate(`/events/${id}`)} className="btn btn-secondary">
              Cancel
            </button>
          </div>
        </form>
      ...
      {/* Score clearing confirmation modal */}
      {showScoreWarning && (
        <div style={{
          position: 'fixed',
          top: 0,
