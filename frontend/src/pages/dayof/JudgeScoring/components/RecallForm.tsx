...
const RecallForm = ({
  ...
  // Use 2-column grid when 6+ couples for density on small screens
  const useGrid = couples.length >= 6;
  ...
  return (
    <div>
      ...
      <div className={useGrid ? 'grid grid-cols-2 gap-1' : ''}>
        {couples.map(couple => {
          ...
          return (
            <div
              ...
              className={`flex items-center gap-2 px-2.5 py-2 rounded-md transition-all select-none touch-manipulation [-webkit-tap-highlight-color:transparent] min-h-[44px] ${
                !useGrid ? 'mb-[0.3125rem]' : ''
              } ${
                selected
